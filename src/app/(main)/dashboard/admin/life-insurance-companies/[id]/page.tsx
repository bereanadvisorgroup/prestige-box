...
interface LifeInsuranceCompanyPageProps {
  params: {
    id: string;
  };
}

export default async function LifeInsuranceCompanyPage({ params }: LifeInsuranceCompanyPageProps) {
  ...
  const linkedPolicies = allPolicies.filter((p) => p.lifeInsuranceCompanyId === company.id);
              ...
              {associatedCompanies.length > 0 ? (
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/admin/life-insurance-companies/${company.id}/edit`}>Link Companies</Link>
                  </Button>
                </div>
              )}
            </CardContent>
