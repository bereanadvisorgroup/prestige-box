...
import { createMoneyManager, updateMoneyManager } from "@/actions/money-managers";
...
interface MoneyManagerFormProps {
  moneyManager?: MoneyManager;
}

export function MoneyManagerForm({ moneyManager }: MoneyManagerFormProps) {
  ...
  const form = useForm<MoneyManagerFormInput, any, MoneyManagerFormValues>({
    ...
    defaultValues: moneyManager
      ? {
          personTitles: moneyManager.personTitles || {},
          id: moneyManager.id,
  ...
  const handleAddPerson = (personId: string) => {
    ...
    if (!current.includes(personId)) {
      ...
      toast.error("This person is already associated with this money manager");
    }
  };
  ...
  async function onSubmit(values: MoneyManagerFormValues) {
    try {
      ...
      if (isEditing) {
        result = await updateMoneyManager(moneyManager.id!, values);
      } else {
        result = await createMoneyManager(values);
      }

      if (result.success) {
        toast.success(isEditing ? "Money Manager record updated" : "Money Manager record created");
        router.push("/dashboard/admin/money-managers");
        router.refresh();
      } else {
        toast.error(result.error || `Failed to ${isEditing ? "update" : "create"} money manager record`);
      }
    } catch (error) {
    ...
    <Card className="mx-auto w-full max-w-3xl shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {moneyManager ? "Edit Money Manager" : "Add Money Manager"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Money Manager Name</FormLabel>
                  <FormControl>
                    <Input placeholder="Apex Wealth Management" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Add notes about this money manager..."
                      {...field}
                      value={field.value || ""}
                    />
                  </FormControl>
                ...
                <FormItem className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 pt-2">
                      <FormLabel>Add Professional to Money Manager</FormLabel>
                      <PersonSearchSelect
            ...
            <div className="flex justify-end gap-3 border-t pt-6 font-semibold">
              ...
              <Button type="submit" disabled={isLoading} className="font-bold">
                {isLoading ? "Saving..." : moneyManager ? "Update Money Manager" : "Create Money Manager"}
              </Button>
            </div>
          </form>
        </Form>
