...
export type EnrichedMoneyManager = MoneyManager & {
  isLinked?: boolean;
};

export const columns = (onDelete: (moneyManager: MoneyManager) => void) => [
  {
  ...
  {
    id: "actions",
    cell: ({ row }: { row: Row<EnrichedMoneyManager> }) => {
      const moneyManager = row.original;
      const isDeletable = !moneyManager.isLinked;
