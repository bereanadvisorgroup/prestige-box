...
import { deleteFinancialAccountType } from "@/actions/financial-account-types";
...
import { columns, type EnrichedFinancialAccountType } from "./columns";
import { DeleteAccountTypeAlert } from "./delete-account-type-alert";

interface AccountTypesTableProps {
  data: EnrichedFinancialAccountType[];
}

export function AccountTypesTable({ data }: AccountTypesTableProps) {
  const [isDeleting, setIsDeleting] = useState(false);
  const [typeToDelete, setTypeToDelete] = useState<FinancialAccountType | null>(null);
  const [searchValue, setSearchValue] = useState("");

  const tableColumns = useMemo(() => columns(setTypeToDelete), []);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteFinancialAccountType(typeToDelete.id);
      if (result.success) {
        toast.success("Financial Account Type deleted successfully");
    ...
    <div className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        ...
        <Button asChild className="shrink-0 font-semibold shadow-sm">
          <Link href="/dashboard/admin/financial-account-types/new">
      ...
      <DeleteAccountTypeAlert
        isOpen={!!typeToDelete}
        onClose={() => setTypeToDelete(null)}
        onConfirm={handleDelete}
        isLoading={isDeleting}
        accountTypeName={typeToDelete?.name || ""}
      />
    </div>
  );
}
