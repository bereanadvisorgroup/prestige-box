...
export function AccountantForm({ accountant }: AccountantFormProps) {
  ...
  async function onSubmit(values: Accountant) {
    try {
      setIsLoading(true);
      const isEditing = !!accountant?.id;

      let result;
      if (isEditing) {
        result = await updateAccountant(accountant.id!, values);
      } else {
        result = await createAccountant(values);
      }

      if (result.success) {
        toast.success(isEditing ? "Accountant record updated" : "Accountant record created");
        router.push("/dashboard/crm/accountants");
        router.refresh();
      } else {
        toast.error(result.error || `Failed to ${isEditing ? "update" : "create"} accountant record`);
      }
    } catch (error) {
    ...
    <Card className="w-full max-w-3xl mx-auto shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {accountant ? "Edit Accountant" : "Add Accountant"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  ...
                  />
                  <FormDescription>Select the person record for this accountant.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <div className="flex justify-end gap-3 pt-6 border-t font-semibold">
              ...
              <Button type="submit" disabled={isLoading} className="font-bold">
                {isLoading ? "Saving..." : accountant ? "Update Accountant" : "Create Accountant"}
              </Button>
            </div>
          </form>
        </Form>
