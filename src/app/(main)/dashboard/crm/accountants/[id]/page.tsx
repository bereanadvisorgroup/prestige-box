...
interface AccountantDetailsPageProps {
  params: {
    id: string;
  };
}
...
export default async function AccountantDetailsPage({ params }: AccountantDetailsPageProps) {
  ...
  const associatedClients = allClients.filter((c) => (accountant.clientIds || []).includes(c.id!));
            ...
            {associatedClients.length > 0 ? (
              ...
              <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                ...
                <Button asChild variant="link" className="mt-2">
                  <Link href={`/dashboard/crm/accountants/${accountant.id}/edit`}>Link Clients</Link>
                </Button>
              </div>
            )}
          </CardContent>
