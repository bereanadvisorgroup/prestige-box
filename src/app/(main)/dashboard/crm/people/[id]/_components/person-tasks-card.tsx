...
import { ArrowUpRight, CheckSquare, Loader2, Plus } from "lucide-react";
...
export function PersonTasksCard({ personId, initialTasks = [], onViewAllClick }: PersonTasksCardProps) {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    let targetAssigneeId = assigneeId || profile?.uid;
    if (!targetAssigneeId) {
      const uRes = await getUsers();
      if (uRes.success && uRes.users && uRes.users.length > 0) {
        targetAssigneeId = uRes.users[0].uid;
      }
    }
    ...
    try {
      ...
      const res = await createTask({
        ...
        associations: [{ entityType: "person", entityId: personId }],
        attachments: [],
      });
      ...
      if (res.success) {
        ...
        setTaskName("");
        const fresh = await getTasks({ personId });
        if (fresh.success && fresh.tasks) {
        ...
        <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-1 max-h-[160px] scrollbar-thin">
          {activeTasks.length > 0 ? (
            activeTasks.map((task) => (
              <Link
                key={task.id}
                href={`/dashboard/crm/tasks?editTask=${task.id}`}
