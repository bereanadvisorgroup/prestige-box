...
interface PersonNotebookButtonProps {
  person: Person;
}

export function PersonNotebookButton({ person }: PersonNotebookButtonProps) {
  const router = useRouter();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [urlInput, setUrlInput] = useState(person.notebookUrl || "");
  const [error, setError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);

  const hasUrl = !!person.notebookUrl;
  ...
  const handleLeftClick = (e: React.MouseEvent) => {
    ...
    window.open(person.notebookUrl!, "_blank", "noopener,noreferrer");
  };

  const handleRightClick = (e: React.MouseEvent) => {
    e.preventDefault();
    setUrlInput(person.notebookUrl || "");
    setError(null);
    setDialogOpen(true);
  };
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent side="bottom" align="start" className="max-w-xs text-xs">
            Left Click to navigate to person notebook, Right Click to edit notebook link
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the URL for the person's notebook. Left-clicking the Notebook button will open this link in a new
              tab.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="person-notebook-url">Notebook URL</Label>
              <Input
                id="person-notebook-url"
                type="text"
