...
interface PersonDocumentsButtonProps {
  person: Person;
}

export function PersonDocumentsButton({ person }: PersonDocumentsButtonProps) {
  const router = useRouter();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [urlInput, setUrlInput] = useState(person.documentUrl || "");
  const [error, setError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);

  const hasUrl = !!person.documentUrl;
  ...
  const handleLeftClick = (e: React.MouseEvent) => {
    ...
    window.open(person.documentUrl!, "_blank", "noopener,noreferrer");
  };

  const handleRightClick = (e: React.MouseEvent) => {
    e.preventDefault();
    setUrlInput(person.documentUrl || "");
    setError(null);
    setDialogOpen(true);
  };
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent side="bottom" align="start" className="max-w-xs text-xs">
            Left Click to navigate to person documents, Right Click to edit document link
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the URL for the person's documents. Left-clicking the Documents button will open this link in a new
              tab.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="person-document-url">Document URL</Label>
              <Input
