...
export type EnrichedInsuranceAgency = InsuranceAgency & {
  isLinked?: boolean;
};

export const columns = (onDelete: (agency: InsuranceAgency) => void): ColumnDef<InsuranceAgency>[] => [
  {
    accessorKey: "firmName",
    filterFn: "includesString",
    header: ({ column }) => <DataTableColumnHeader column={column} title="Agency Name" />,
    cell: ({ row }: { row: Row<InsuranceAgency> }) => {
      const agency = row.original;
      return (
        <div className="flex items-center gap-2">
          <FirmLogo logoUrl={agency.logoUrl} name={agency.firmName} className="h-6 w-6" />
          <Link
            href={`/dashboard/crm/insurance-agencies/${agency.id}`}
            className="flex items-center gap-1 font-medium text-primary hover:underline"
          >
            <span className="truncate">{agency.firmName}</span>
  ...
  {
    id: "actions",
    cell: ({ row }: { row: Row<EnrichedInsuranceAgency> }) => {
      const agency = row.original;
      const isDeletable = !agency.isLinked;

      return (
        <div className="flex items-center justify-end gap-2">
          <Link href={`/dashboard/crm/insurance-agencies/${agency.id}/edit`}>
