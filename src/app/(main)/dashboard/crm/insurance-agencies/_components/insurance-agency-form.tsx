...
import { Globe, MapPin, Phone, Shield, Trash2, Users } from "lucide-react";
...
import { createInsuranceAgency, updateInsuranceAgency } from "@/actions/insurance-agencies";
...
import {
  ...
  type InsuranceAgencyFormInput,
  InsuranceAgencyFormSchema,
  type InsuranceAgencyFormValues,
  type Person,
} from "@/types/crm";

interface InsuranceAgencyFormProps {
  insuranceAgency?: InsuranceAgency;
}

export function InsuranceAgencyForm({ insuranceAgency }: InsuranceAgencyFormProps) {
  ...
  const form = useForm<InsuranceAgencyFormInput, any, InsuranceAgencyFormValues>({
    resolver: zodResolver(InsuranceAgencyFormSchema),
    defaultValues: insuranceAgency
      ? {
          id: insuranceAgency.id,
          personIds: insuranceAgency.personIds,
          personTitles: insuranceAgency.personTitles || {},
          firmName: insuranceAgency.firmName,
          firmAddressId: insuranceAgency.firmAddressId,
          website: insuranceAgency.website,
          phone: insuranceAgency.phone,
          clientIds: insuranceAgency.clientIds,
          companyIds: insuranceAgency.companyIds,
          logoUrl: insuranceAgency.logoUrl || null,
        }
      : {
          personIds: [],
          personTitles: {},
  ...
  const handleAddPerson = (personId: string) => {
    ...
    if (!current.includes(personId)) {
      ...
      toast.error("This person is already associated with this insurance agency");
    }
  };
  ...
  async function onSubmit(values: InsuranceAgencyFormValues) {
    try {
      setIsLoading(true);
      const isEditing = !!insuranceAgency?.id;

      const result = isEditing
        ? await updateInsuranceAgency(insuranceAgency.id!, values)
        : await createInsuranceAgency(values);

      if (result.success) {
        toast.success(isEditing ? "Insurance Agency record updated" : "Insurance Agency record created");
        window.dispatchEvent(new CustomEvent("association-change"));
        router.push("/dashboard/crm/insurance-agencies");
        router.refresh();
      } else {
        toast.error(result.error || `Failed to ${isEditing ? "update" : "create"} insurance agency record`);
      }
    } catch (error) {
    ...
    <Card className="mx-auto w-full max-w-3xl shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {insuranceAgency ? "Edit Insurance Agency" : "Add Insurance Agency"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <FormItem>
                  <FormControl>
                    <LogoUpload
                      ...
                      entityId={insuranceAgency?.id}
                      entityType="insurance-agencies"
                      name={form.watch("firmName") || "Agency"}
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Insurance Agency Name</FormLabel>
                  <FormControl>
                    <Input placeholder="Apex Financial & Insurance Services" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                ...
                <FormItem className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 pt-2">
                      <FormLabel>Add Professional to Insurance Agency</FormLabel>
                      <PersonSearchSelect
                        ...
                        value=""
                      />
                      <FormDescription>Select professionals to add them to this agency.</FormDescription>
                    </div>
                  </div>
                  <FormMessage />
                </FormItem>
            ...
            <div className="flex justify-end gap-3 border-t pt-6 font-semibold">
              ...
              <Button type="submit" disabled={isLoading} className="font-bold">
                {isLoading ? "Saving..." : insuranceAgency ? "Update Insurance Agency" : "Create Insurance Agency"}
              </Button>
            </div>
          </form>
        </Form>
