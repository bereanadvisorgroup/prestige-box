...
import { deleteInsuranceAgency } from "@/actions/insurance-agencies";
...
import type { InsuranceAgency } from "@/types/crm";

import { columns } from "./columns";
import { DeleteInsuranceAgencyAlert } from "./delete-insurance-agency-alert";

interface InsuranceAgenciesTableProps {
  data: InsuranceAgency[];
}

export function InsuranceAgenciesTable({ data }: InsuranceAgenciesTableProps) {
  const [isDeleting, setIsDeleting] = useState(false);
  const [agencyToDelete, setAgencyToDelete] = useState<InsuranceAgency | null>(null);
  const [searchValue, setSearchValue] = useState("");

  const tableColumns = useMemo(() => columns(setAgencyToDelete), []);

  const handleDelete = async () => {
    if (!agencyToDelete?.id) return;

    setIsDeleting(true);
    try {
      const result = await deleteInsuranceAgency(agencyToDelete.id);
      if (result.success) {
        toast.success("Insurance Agency deleted successfully");
        setAgencyToDelete(null);
      } else {
        toast.error(result.error || "Failed to delete insurance agency");
      }
    } catch (_error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        ...
        <Button asChild className="shrink-0 font-semibold shadow-sm">
          <Link href="/dashboard/crm/insurance-agencies/new">
