...
interface InsuranceAgencyDetailsPageProps {
  params: Promise<{
    id: string;
  }>;
}

export default async function InsuranceAgencyDetailsPage({ params }: InsuranceAgencyDetailsPageProps) {
  ...
  const { insuranceAgency, people: peopleRaw, address: addressRaw } = result;
  ...
  const associatedClients = allClients.filter((c) => (insuranceAgency.clientIds || []).includes(c.id!));
  ...
  const associatedCompanies = allCompanies.filter((c) => (insuranceAgency.companyIds || []).includes(c.id!));
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 md:px-6">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div className="flex items-center gap-4">
          <FirmLogo
            logoUrl={insuranceAgency.logoUrl}
            name={insuranceAgency.firmName}
            className="h-16 w-16 rounded-md border-2 border-primary/10"
            size="lg"
          />
          <div>
            <h1 className="font-bold text-3xl tracking-tight">{insuranceAgency.firmName}</h1>
        ...
        <Button asChild variant="outline" className="font-semibold shadow-sm">
          <Link href={`/dashboard/crm/insurance-agencies/${insuranceAgency.id}/edit`}>
            <Edit className="mr-2 h-4 w-4" />
            Edit Insurance Agency
          </Link>
        </Button>
      </div>
          ...
          <CardContent className="space-y-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-muted-foreground text-xs">
                            {(insuranceAgency.personTitles as Record<string, string>)?.[person.id as string] ||
            ...
            <div className="flex items-start gap-3 pt-2">
              ...
              <div className="space-y-1">
                <p className="font-medium text-sm leading-none">Agency Address</p>
                {address ? (
              ...
              <>
                ...
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <a
                      href={
                        insuranceAgency.website.startsWith("http")
                          ? insuranceAgency.website
                          : `https://${insuranceAgency.website}`
                      }
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 text-blue-600 text-sm hover:underline"
                    >
                      {insuranceAgency.website.replace(/^https?:\/\//, "")}
                      <ArrowUpRight className="h-3 w-3 opacity-60" />
                    </a>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <a href={`tel:${insuranceAgency.phone}`} className="text-primary text-sm hover:underline">
                      {formatPhoneNumber(insuranceAgency.phone)}
                    </a>
                  </div>
                </div>
              </>
              ...
              {associatedClients.length > 0 ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {associatedClients.map((client) => {
                    ...
                    return (
                      <Link
                        key={client.id}
                        href={`/dashboard/crm/clients/${client.id}/insurance-agencies`}
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/insurance-agencies/${insuranceAgency.id}/edit`}>Link Clients</Link>
                  </Button>
                </div>
              )}
            </CardContent>
            ...
            <CardContent>
              {associatedCompanies.length > 0 ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {associatedCompanies.map((company) => {
                    ...
                    return (
                      <Link
                        key={company.id}
                        href={`/dashboard/crm/companies/${company.id}/insurance-agencies`}
                        className="group flex items-center justify-between rounded-lg border p-4 transition-colors hover:bg-muted/50"
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 font-semibold text-sm transition-colors group-hover:text-primary">
                          ...
                          <div className="text-muted-foreground text-xs">
                            {companyWebsite && <span className="mr-3">{companyWebsite}</span>}
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/insurance-agencies/${insuranceAgency.id}/edit`}>Link Companies</Link>
                  </Button>
                </div>
              )}
            </CardContent>
