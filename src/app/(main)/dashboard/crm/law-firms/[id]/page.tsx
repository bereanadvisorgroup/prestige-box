...
export default async function LawFirmDetailsPage({ params }: LawFirmDetailsPageProps) {
  ...
  const { lawFirm, people: peopleRaw, address: addressRaw } = result;
  ...
  const associatedClients = allClients.filter((c) => (lawFirm.clientIds || []).includes(c.id!));
  ...
  const associatedCompanies = allCompanies.filter((c) => (lawFirm.companyIds || []).includes(c.id!));
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 md:px-6">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div className="flex items-center gap-4">
          <FirmLogo
            logoUrl={lawFirm.logoUrl}
            name={lawFirm.firmName}
            className="h-16 w-16 rounded-md border-2 border-primary/10"
            size="lg"
          />
          <div>
            <h1 className="font-bold text-3xl tracking-tight">{lawFirm.firmName}</h1>
              ...
              <>
                ...
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <a href={`tel:${lawFirm.phone}`} className="text-primary text-sm hover:underline">
              ...
              {associatedClients.length > 0 ? (
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/law-firms/${lawFirm.id}/edit`}>Link Clients</Link>
                  </Button>
                </div>
              )}
            </CardContent>
            ...
            <CardContent>
              {associatedCompanies.length > 0 ? (
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/law-firms/${lawFirm.id}/edit`}>Link Companies</Link>
                  </Button>
                </div>
              )}
            </CardContent>
