...
export default async function CRMOverviewPage() {
  ...
  const stats = {
    ...
    monthlyRevenue: paymentsResult.success ? (paymentsResult.payments || []).reduce((acc, p) => acc + p.paymentAmount, 0) : 0,
    ...
    <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto py-8 px-4 md:px-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Real-time insights across your client base and policy portfolio.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border-l-4 border-l-blue-500">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Total Profiles</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <span className="text-3xl font-bold">{stats.people}</span>
              <div className="bg-blue-50 p-2 rounded-lg"><Users className="h-5 w-5 text-blue-500" /></div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-purple-500">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Households</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <span className="text-3xl font-bold">{stats.households}</span>
              <div className="bg-purple-50 p-2 rounded-lg"><Home className="h-5 w-5 text-purple-500" /></div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-amber-500">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Client Records</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <span className="text-3xl font-bold">{stats.clients}</span>
              <div className="bg-amber-50 p-2 rounded-lg"><Briefcase className="h-5 w-5 text-amber-500" /></div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-emerald-500 bg-emerald-50/10">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Monthly Projection</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <Badge variant="outline" className="text-green-600 border-green-200 bg-green-50 font-bold uppercase text-[10px]">HEALTHY</Badge>
                </div>
