...
export default async function AccountingFirmDetailsPage({ params }: AccountingFirmDetailsPageProps) {
  ...
  const associatedCompanies = allCompanies.filter((c) => (accountingFirm.companyIds || []).includes(c.id!));
            ...
            <CardContent>
              {associatedCompanies.length > 0 ? (
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/accounting-firms/${accountingFirm.id}/edit`}>Link Companies</Link>
                  </Button>
                </div>
              )}
            </CardContent>
