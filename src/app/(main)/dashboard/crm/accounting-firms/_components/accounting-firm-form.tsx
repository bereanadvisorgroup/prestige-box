...
import { Globe, MapPin, Phone, ReceiptText, Trash2, Users } from "lucide-react";
import { useForm } from "react-hook-form";
import { toast } from "sonner";

import { createAccountingFirm, updateAccountingFirm } from "@/actions/accounting-firms";
...
import { type AccountingFirm, AccountingFirmSchema, type Address, type Client, type Person } from "@/types/crm";

interface AccountingFirmFormProps {
  accountingFirm?: AccountingFirm;
}

export function AccountingFirmForm({ accountingFirm }: AccountingFirmFormProps) {
  ...
  const handleAddPerson = (personId: string) => {
    ...
    if (!current.includes(personId)) {
      ...
      toast.error("This person is already associated with this accounting firm");
    }
  };
  ...
  async function onSubmit(values: AccountingFirm) {
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || `Failed to ${isEditing ? "update" : "create"} accounting firm record`);
      }
    } catch (error) {
    ...
    <Card className="mx-auto w-full max-w-3xl shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {accountingFirm ? "Edit Accounting Firm" : "Add Accounting Firm"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Precision Tax & Audit" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                ...
                <FormItem className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 pt-2">
                      <FormLabel>Add Professional to Accounting Firm</FormLabel>
                      <PersonSearchSelect
            ...
            <div className="flex justify-end gap-3 border-t pt-6 font-semibold">
              ...
              <Button type="submit" disabled={isLoading} className="font-bold">
                {isLoading ? "Saving..." : accountingFirm ? "Update Accounting Firm" : "Create Accounting Firm"}
              </Button>
            </div>
          </form>
        </Form>
