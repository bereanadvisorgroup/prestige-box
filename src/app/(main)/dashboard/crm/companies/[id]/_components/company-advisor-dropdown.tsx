...
interface CompanyAdvisorDropdownProps {
  company: Company;
  advisors: {
    uid: string;
    firstName: string;
...
export function CompanyAdvisorDropdown({ company, advisors }: CompanyAdvisorDropdownProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [currentAdvisorId, setCurrentAdvisorId] = useState<string>(company.advisorId || "none");
  ...
  const handleAdvisorChange = async (value: string) => {
    ...
    startTransition(async () => {
      try {
        const res = await updateCompany(company.id!, { advisorId: nextValue });
        if (res.success) {
