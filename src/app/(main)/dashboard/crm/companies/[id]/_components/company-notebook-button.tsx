...
interface CompanyNotebookButtonProps {
  company: Company;
}

export function CompanyNotebookButton({ company }: CompanyNotebookButtonProps) {
  const router = useRouter();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [urlInput, setUrlInput] = useState(company.notebookUrl || "");
  const [error, setError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);

  const hasUrl = !!company.notebookUrl;
  ...
  const handleLeftClick = (e: React.MouseEvent) => {
    ...
    window.open(company.notebookUrl!, "_blank", "noopener,noreferrer");
  };

  const handleRightClick = (e: React.MouseEvent) => {
    e.preventDefault();
    setUrlInput(company.notebookUrl || "");
    setError(null);
    setDialogOpen(true);
  };
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent side="bottom" align="start" className="max-w-xs text-xs">
            Left Click to navigate to company notebook, Right Click to edit notebook link
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the URL for the company's notebook. Left-clicking the Notebook button will open this link in a new
              tab.
            </DialogDescription>
          </DialogHeader>
