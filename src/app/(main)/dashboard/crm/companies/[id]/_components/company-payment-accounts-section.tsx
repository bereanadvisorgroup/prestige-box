...
import type { Bank, Company, PaymentAccount } from "@/types/crm";

export function CompanyPaymentAccountsSection({
  company,
  associatedBanks,
}: {
  company: Company;
  ...
  const [paymentAccounts, setPaymentAccounts] = useState<PaymentAccount[]>(company.paymentAccounts || []);
  ...
  const handleUpdate = async (updates: Partial<Company>) => {
    try {
      setIsLoading(true);
      const res = await updateCompany(company.id!, updates);
      if (res.success) {
  ...
  return (
    <Card className="border-none bg-gradient-to-b from-card to-muted/20 shadow-md mt-6">
