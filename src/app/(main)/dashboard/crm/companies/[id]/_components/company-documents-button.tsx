...
interface CompanyDocumentsButtonProps {
  company: Company;
}

export function CompanyDocumentsButton({ company }: CompanyDocumentsButtonProps) {
  const router = useRouter();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [urlInput, setUrlInput] = useState(company.documentUrl || "");
  const [error, setError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);

  const hasUrl = !!company.documentUrl;
  ...
  const handleLeftClick = (e: React.MouseEvent) => {
    ...
    window.open(company.documentUrl!, "_blank", "noopener,noreferrer");
  };

  const handleRightClick = (e: React.MouseEvent) => {
    e.preventDefault();
    setUrlInput(company.documentUrl || "");
    setError(null);
    setDialogOpen(true);
  };
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent side="bottom" align="start" className="max-w-xs text-xs">
            Left Click to navigate to company documents, Right Click to edit document link
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the URL for the company's documents. Left-clicking the Documents button will open this link in a new
              tab.
            </DialogDescription>
          </DialogHeader>
