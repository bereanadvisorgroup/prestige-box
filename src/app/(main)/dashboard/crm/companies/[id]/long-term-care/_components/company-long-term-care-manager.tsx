...
import {
  linkCompanyToLongTermCareInsurance,
  unlinkCompanyFromLongTermCareInsurance,
...
import type { ClientDocument, Company, LongTermCareInsurance } from "@/types/crm";

interface CompanyLongTermCareManagerProps {
  company: Company;
  allCompanies: LongTermCareInsurance[];
}

const DOCUMENT_TYPES = ["LTC Policy", "Benefit Summary", "Policy Statement", "Other"];

export function CompanyLongTermCareManager({ company, allCompanies }: CompanyLongTermCareManagerProps) {
  const router = useRouter();
  const [_isPending, startTransition] = useTransition();

  // Local document state
  const [documents, setDocuments] = useState<ClientDocument[]>(company.ltcDocuments || []);
  ...
  const associatedCompanies = useMemo(() => {
    return allCompanies.filter((c) => c.companyIds?.includes(company.id || ""));
  }, [allCompanies, company.id]);
  ...
  // Documents grouped by firmId (firmId field is used inside ClientDocument to link to the insurance company)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const filePath = `companies/${company.id}/ltcDocuments/${randomStr}_${Date.now()}.${fileExt}`;
      ...
      const res = await updateCompany(company.id!, { ltcDocuments: updatedDocs });
      if (!res.success) throw new Error(res.error || "Failed to save company documents");
      ...
      if (addingCompanyId && !associatedCompanies.some((c) => c.id === addingCompanyId)) {
        const linkRes = await linkCompanyToLongTermCareInsurance(addingCompanyId, company.id!);
        if (linkRes.success) {
  ...
  const handleLinkCompany = async () => {
    ...
    setIsLinking(true);
    try {
      const result = await linkCompanyToLongTermCareInsurance(linkCompanyId, company.id!);
      if (result.success) {
        toast.success("Long-Term Care Insurance Company linked successfully");
        window.dispatchEvent(new CustomEvent("association-change"));
        setIsLinkOpen(false);
        setLinkCompanyId("");
        startTransition(() => {
  ...
  const handleUnlinkCompany = async (insuranceCompanyId: string) => {
    const confirmUnlink = window.confirm(
      "Are you sure you want to remove association with this long-term care insurance company?",
    );
    if (!confirmUnlink) return;

    try {
      const result = await unlinkCompanyFromLongTermCareInsurance(insuranceCompanyId, company.id!);
      if (result.success) {
  ...
  const handleDeleteDocument = async (docId: string) => {
    ...
    try {
      ...
      const res = await updateCompany(company.id!, { ltcDocuments: updated });
      if (res.success) {
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <h1 className="flex items-center gap-2 font-bold text-foreground text-xl tracking-tight">
              <HeartPulse className="h-5 w-5 text-primary" /> Long-Term Care Insurance
            </h1>
            <p className="text-muted-foreground text-sm">
              Manage company's long-term care insurance connections and documents in one consolidated place.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          <h2 className="font-semibold text-muted-foreground text-sm uppercase tracking-wider">
            Associated Long-Term Care Companies
          </h2>

          {associatedCompanies.length > 0 ? (
            associatedCompanies.map((c) => {
              const companyDocs = documentsByCompany.get(c.id!) || [];
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Link
                            href={`/dashboard/admin/long-term-care-insurance/${c.id}`}
            ...
            <Card className="flex flex-col items-center justify-center border-2 border-muted/30 border-dashed bg-muted/5 px-6 py-12 text-center text-muted-foreground">
              ...
              <p className="mt-1 max-w-sm text-xs">
                Connect this company to a long-term care insurance company to manage documents and associations.
              </p>
            ...
            <div>
              ...
              <Card className="border border-muted/20 bg-gradient-to-b from-card to-muted/5 shadow-sm">
                <CardHeader className="py-4">
                  ...
                  <CardDescription className="text-xs">Companies you can associate with this company.</CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="max-h-60 divide-y divide-muted/10 overflow-y-auto">
                    {availableCompaniesToLink.map((c) => (
                      <div key={c.id ?? c.name} className="flex items-center justify-between p-3.5 text-xs">
                        <div className="min-w-0 pr-2">
                          <p className="truncate font-semibold text-foreground">{c.name}</p>
                          {c.websiteUrl && <p className="truncate text-[10px] text-muted-foreground">{c.websiteUrl}</p>}
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={async () => {
                            setIsLinking(true);
                            try {
                              const res = await linkCompanyToLongTermCareInsurance(c.id!, company.id!);
                              if (res.success) {
        ...
        <DialogContent className="border border-muted/20 bg-background sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <UploadCloud className="h-5 w-5 text-primary" /> Add LTC Document
            </DialogTitle>
            <DialogDescription>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        ...
                        {allCompanies.map((c) => (
                          ...
                          >
                            <Check
                              className={cn("mr-2 h-4 w-4 opacity-0", addingCompanyId === c.id && "opacity-100")}
                            />
                            {c.name}
                          </CommandItem>
                        ))}
      ...
      <Dialog open={isLinkOpen} onOpenChange={setIsLinkOpen}>
        <DialogContent className="border border-muted/20 bg-background sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Building2 className="h-5 w-5 text-primary" /> Link LTC Company
            </DialogTitle>
            <DialogDescription>Link an existing long-term care insurance company to this company.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
                      ...
                      <CommandGroup>
                        {availableCompaniesToLink.map((c) => (
                          <CommandItem
                            ...
                            value={c.name}
                            onSelect={() => {
                              setLinkCompanyId(c.id || "");
                              setLinkComboboxOpen(false);
                            }}
                          >
                            <Check className={cn("mr-2 h-4 w-4 opacity-0", linkCompanyId === c.id && "opacity-100")} />
                            {c.name}
                          </CommandItem>
                        ))}
                      </CommandGroup>
            ...
            <Button variant="outline" asChild className="w-full">
              <Link href={`/dashboard/admin/long-term-care-insurance/new?companyId=${company.id}`}>
                Create New Company
              </Link>
            </Button>
          </div>
