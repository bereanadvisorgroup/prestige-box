...
import {
  linkCompanyToDisabilityInsuranceCompany,
  unlinkCompanyFromDisabilityInsuranceCompany,
...
import type { ClientDocument, Company, DisabilityInsuranceCompany } from "@/types/crm";

interface CompanyDisabilityInsuranceManagerProps {
  company: Company;
...
export function CompanyDisabilityInsuranceManager({ company, allCompanies }: CompanyDisabilityInsuranceManagerProps) {
  const router = useRouter();
  const [_isPending, startTransition] = useTransition();

  // Local document state
  const [documents, setDocuments] = useState<ClientDocument[]>(company.disabilityDocuments || []);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const filePath = `companies/${company.id}/disabilityDocuments/${randomStr}_${Date.now()}.${fileExt}`;
      ...
      const res = await updateCompany(company.id!, { disabilityDocuments: updatedDocs });
      ...
      if (addingCompanyId && !associatedCompanies.some((c) => c.id === addingCompanyId)) {
        const linkRes = await linkCompanyToDisabilityInsuranceCompany(addingCompanyId, company.id!);
        if (linkRes.success) {
  ...
  const handleLinkCompany = async () => {
    ...
    setIsLinking(true);
    try {
      const result = await linkCompanyToDisabilityInsuranceCompany(linkCompanyId, company.id!);
      if (result.success) {
  ...
  const handleUnlinkCompany = async (companyId: string) => {
    ...
    try {
      const result = await unlinkCompanyFromDisabilityInsuranceCompany(companyId, company.id!);
      if (result.success) {
  ...
  const handleDeleteDocument = async (docId: string) => {
    ...
    try {
      ...
      const res = await updateCompany(company.id!, { disabilityDocuments: updated });
      if (res.success) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm">
              Manage company's disability insurance company connections and documents in one consolidated place.
            </p>
          </div>
          ...
          {associatedCompanies.length > 0 ? (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Link
                            href={`/dashboard/admin/disability-insurance-companies/${c.id}`}
            ...
            <Card className="flex flex-col items-center justify-center border-2 border-muted/30 border-dashed bg-muted/5 px-6 py-12 text-center text-muted-foreground">
              ...
              <p className="mt-1 max-w-sm text-xs">
                Connect this company to a disability insurance company to manage documents and associations.
              </p>
                      ...
                      <div key={c.id ?? c.name} className="flex items-center justify-between p-3.5 text-xs">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            setIsLinking(true);
                            try {
                              const res = await linkCompanyToDisabilityInsuranceCompany(c.id!, company.id!);
                              if (res.success) {
      ...
      <Dialog open={isLinkOpen} onOpenChange={setIsLinkOpen}>
        <DialogContent className="border border-muted/20 bg-background sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Link an existing disability insurance company to this company.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <Button variant="outline" asChild className="w-full">
              <Link href={`/dashboard/admin/disability-insurance-companies/new?companyId=${company.id}`}>
                Create New Company
              </Link>
            </Button>
          </div>
