...
import {
  linkCompanyToLifeInsuranceCompany,
  unlinkCompanyFromLifeInsuranceCompany,
...
import type { ClientDocument, Company, LifeInsuranceCompany } from "@/types/crm";

interface CompanyLifeInsuranceManagerProps {
  company: Company;
...
export function CompanyLifeInsuranceManager({ company, allCompanies }: CompanyLifeInsuranceManagerProps) {
  const router = useRouter();
  const [_isPending, startTransition] = useTransition();

  // Local document state
  const [documents, setDocuments] = useState<ClientDocument[]>(company.lifeDocuments || []);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const filePath = `companies/${company.id}/lifeDocuments/${randomStr}_${Date.now()}.${fileExt}`;
      ...
      const res = await updateCompany(company.id!, { lifeDocuments: updatedDocs });
      ...
      if (addingCompanyId && !associatedCompanies.some((c) => c.id === addingCompanyId)) {
        const linkRes = await linkCompanyToLifeInsuranceCompany(addingCompanyId, company.id!);
        if (linkRes.success) {
  ...
  const handleLinkCompany = async () => {
    ...
    setIsLinking(true);
    try {
      const result = await linkCompanyToLifeInsuranceCompany(linkCompanyId, company.id!);
      if (result.success) {
  ...
  const handleUnlinkCompany = async (companyId: string) => {
    ...
    try {
      const result = await unlinkCompanyFromLifeInsuranceCompany(companyId, company.id!);
      if (result.success) {
  ...
  const handleDeleteDocument = async (docId: string) => {
    ...
    try {
      ...
      const res = await updateCompany(company.id!, { lifeDocuments: updated });
      if (res.success) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm">
              Manage company's life insurance company connections and documents in one consolidated place.
            </p>
          </div>
          ...
          {associatedCompanies.length > 0 ? (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Link
                            href={`/dashboard/admin/life-insurance-companies/${c.id}`}
            ...
            <Card className="flex flex-col items-center justify-center border-2 border-muted/30 border-dashed bg-muted/5 px-6 py-12 text-center text-muted-foreground">
              ...
              <p className="mt-1 max-w-sm text-xs">
                Connect this company to a life insurance company to manage documents and associations.
              </p>
                      ...
                      <div key={c.id ?? c.name} className="flex items-center justify-between p-3.5 text-xs">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            setIsLinking(true);
                            try {
                              const res = await linkCompanyToLifeInsuranceCompany(c.id!, company.id!);
                              if (res.success) {
      ...
      <Dialog open={isLinkOpen} onOpenChange={setIsLinkOpen}>
        <DialogContent className="border border-muted/20 bg-background sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Link an existing life insurance company to this company.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <Button variant="outline" asChild className="w-full">
              <Link href={`/dashboard/admin/life-insurance-companies/new?companyId=${company.id}`}>
                Create New Company
              </Link>
            </Button>
          </div>
