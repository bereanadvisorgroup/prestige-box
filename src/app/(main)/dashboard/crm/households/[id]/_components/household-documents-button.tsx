...
interface HouseholdDocumentsButtonProps {
  household: Household;
}

export function HouseholdDocumentsButton({ household }: HouseholdDocumentsButtonProps) {
  const router = useRouter();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [urlInput, setUrlInput] = useState(household.documentUrl || "");
  const [error, setError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);

  const hasUrl = !!household.documentUrl;
  ...
  const handleLeftClick = (e: React.MouseEvent) => {
    ...
    window.open(household.documentUrl!, "_blank", "noopener,noreferrer");
  };

  const handleRightClick = (e: React.MouseEvent) => {
    e.preventDefault();
    setUrlInput(household.documentUrl || "");
    setError(null);
    setDialogOpen(true);
  };
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent side="bottom" align="start" className="max-w-xs text-xs">
            Left Click to navigate to household documents, Right Click to edit document link
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the URL for the household's documents. Left-clicking the Documents button will open this link in a
              new tab.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="household-document-url">Document URL</Label>
              <Input
                id="household-document-url"
                type="text"
