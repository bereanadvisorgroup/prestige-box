...
import type { Client, ClientDocument, LongTermCareInsurance } from "@/types/crm";
...
export function LongTermCareManager({ client, allCompanies }: LongTermCareManagerProps) {
  const router = useRouter();
  const [_isPending, startTransition] = useTransition();

  // Local document state
  const [documents, setDocuments] = useState<ClientDocument[]>(client.ltcDocuments || []);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const filePath = `clients/${client.id}/ltcDocuments/${randomStr}_${Date.now()}.${fileExt}`;
      ...
      const res = await updateClient(client.id!, { ltcDocuments: updatedDocs });
      ...
      if (addingCompanyId && !associatedCompanies.some((c) => c.id === addingCompanyId)) {
        const linkRes = await linkClientToLongTermCareInsurance(addingCompanyId, client.id!);
        if (linkRes.success) {
  ...
  const handleLinkCompany = async () => {
    ...
    setIsLinking(true);
    try {
      const result = await linkClientToLongTermCareInsurance(linkCompanyId, client.id!);
      if (result.success) {
  ...
  const handleUnlinkCompany = async (companyId: string) => {
    ...
    try {
      const result = await unlinkClientFromLongTermCareInsurance(companyId, client.id!);
      if (result.success) {
  ...
  const handleDeleteDocument = async (docId: string) => {
    ...
    try {
      ...
      const res = await updateClient(client.id!, { ltcDocuments: updated });
      if (res.success) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm">
              Manage client's long-term care insurance connections and documents in one consolidated place.
            </p>
          </div>
          ...
          {associatedCompanies.length > 0 ? (
            ...
            <Card className="flex flex-col items-center justify-center border-2 border-muted/30 border-dashed bg-muted/5 px-6 py-12 text-center text-muted-foreground">
              ...
              <p className="mt-1 max-w-sm text-xs">
                Connect this client to a long-term care insurance company to manage documents and associations.
              </p>
                      ...
                      <div key={company.id ?? company.name} className="flex items-center justify-between p-3.5 text-xs">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            setIsLinking(true);
                            try {
                              const res = await linkClientToLongTermCareInsurance(company.id!, client.id!);
                              if (res.success) {
      ...
      <Dialog open={isLinkOpen} onOpenChange={setIsLinkOpen}>
        <DialogContent className="border border-muted/20 bg-background sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Link an existing long-term care insurance company to this client.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <Button variant="outline" asChild className="w-full">
              <Link href={`/dashboard/admin/long-term-care-insurance/new?clientId=${client.id}`}>
                Create New Company
              </Link>
            </Button>
          </div>
