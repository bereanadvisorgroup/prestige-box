...
export default async function BankDetailsPage({ params }: BankDetailsPageProps) {
  ...
  const { bank, people: peopleRaw, address: addressRaw } = result;
  ...
  const associatedClients = allClients.filter((c) => (bank.clientIds || []).includes(c.id!));
  ...
  const associatedCompanies = allCompanies.filter((c) => (bank.companyIds || []).includes(c.id!));
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 md:px-6">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div className="flex items-center gap-4">
          ...
          />
          <div>
            <h1 className="font-bold text-3xl tracking-tight">{bank.firmName}</h1>
        ...
        <Button asChild variant="outline" className="font-semibold shadow-sm">
          <Link href={`/dashboard/crm/banks/${bank.id}/edit`}>
          ...
          <CardContent className="space-y-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-muted-foreground text-xs">
                            {(bank.personTitles as Record<string, string>)?.[person.id as string] ||
            ...
            <div className="flex items-start gap-3 pt-2">
              ...
              <div className="space-y-1">
                <p className="font-medium text-sm leading-none">Address</p>
                {address ? (
              ...
              <>
                ...
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <a href={`tel:${bank.phone}`} className="text-primary text-sm hover:underline">
                      {formatPhoneNumber(bank.phone)}
                    </a>
                  </div>
                </div>
              </>
              ...
              {associatedClients.length > 0 ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {associatedClients.map((client) => {
                    ...
                    return (
                      <Link
                        key={client.id}
                        href={`/dashboard/crm/clients/${client.id}/banks`}
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/banks/${bank.id}/edit`}>Link Clients</Link>
                  </Button>
                </div>
              )}
            </CardContent>
            ...
            <CardContent>
              {associatedCompanies.length > 0 ? (
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/banks/${bank.id}/edit`}>Link Companies</Link>
                  </Button>
                </div>
              )}
            </CardContent>
