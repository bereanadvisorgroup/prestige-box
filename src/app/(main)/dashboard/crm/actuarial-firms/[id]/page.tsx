...
import { ArrowUpRight, Briefcase, Building2, Calculator, Edit, Globe, Mail, MapPin, Phone, Users } from "lucide-react";
...
interface ActuarialFirmDetailsPageProps {
  params: Promise<{
    id: string;
  }>;
}

export default async function ActuarialFirmDetailsPage({ params }: ActuarialFirmDetailsPageProps) {
  ...
  const { actuarialFirm, people: peopleRaw, address: addressRaw } = result;
  ...
  const associatedClients = allClients.filter((c) => (actuarialFirm.clientIds || []).includes(c.id!));
  ...
  const associatedCompanies = allCompanies.filter((c) => (actuarialFirm.companyIds || []).includes(c.id!));
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 md:px-6">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div className="flex items-center gap-4">
          <FirmLogo
            logoUrl={actuarialFirm.logoUrl}
            name={actuarialFirm.firmName}
            className="h-16 w-16 rounded-md border-2 border-primary/10"
            size="lg"
          />
          <div>
            <h1 className="font-bold text-3xl tracking-tight">{actuarialFirm.firmName}</h1>
        ...
        <Button asChild variant="outline" className="font-semibold shadow-sm">
          <Link href={`/dashboard/crm/actuarial-firms/${actuarialFirm.id}/edit`}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-muted-foreground text-xs">
                            {(actuarialFirm.personTitles as Record<string, string>)?.[person.id as string] ||
              ...
              <>
                ...
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <a
                      href={
                        actuarialFirm.website.startsWith("http")
                          ? actuarialFirm.website
                          : `https://${actuarialFirm.website}`
                      }
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 text-blue-600 text-sm hover:underline"
                    >
                      {actuarialFirm.website.replace(/^https?:\/\//, "")}
                      <ArrowUpRight className="h-3 w-3 opacity-60" />
                    </a>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <a href={`tel:${actuarialFirm.phone}`} className="text-primary text-sm hover:underline">
                      {formatPhoneNumber(actuarialFirm.phone)}
                    </a>
                  </div>
                </div>
              </>
              ...
              {associatedClients.length > 0 ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {associatedClients.map((client) => {
                    ...
                    return (
                      <Link
                        key={client.id}
                        href={`/dashboard/crm/clients/${client.id}/actuarial-firms`}
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/actuarial-firms/${actuarialFirm.id}/edit`}>Link Clients</Link>
                  </Button>
                </div>
              )}
            </CardContent>
            ...
            <CardContent>
              {associatedCompanies.length > 0 ? (
                ...
                <div className="rounded-lg border border-dashed bg-muted/20 py-10 text-center text-muted-foreground">
                  ...
                  <Button asChild variant="link" className="mt-2">
                    <Link href={`/dashboard/crm/actuarial-firms/${actuarialFirm.id}/edit`}>Link Companies</Link>
                  </Button>
                </div>
              )}
            </CardContent>
